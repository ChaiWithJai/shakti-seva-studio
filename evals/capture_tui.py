...
def sanitize(text: str) -> str:
    text = text.replace(str(ROOT), "~/shaki-seva-studio")
...
def capture(timeout: float = 25) -> str:
    ...
    process = subprocess.Popen(
        [str(ROOT / ".venv" / "bin" / "shaki"), "hermes", "--tui"],
        cwd=ROOT,
        stdin=slave,
        stdout=slave,
        stderr=slave,
...
<html lang="en"><meta charset="utf-8"><link rel="icon" href="data:,"><title>Shaki Hermes TUI</title>
<style>
...
</style><body><div class="window"><div class="bar"><i class="dot r"></i><i class="dot y"></i><i class="dot g"></i><span class="title">Shaki Seva Studio · Hermes TUI · governed 32K mode</span></div><pre>{rows}</pre></div><div class="note">Local path and session ID treated for publication.</div></body></html>"""
