...
from .server import ROOT, TRACES
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="shakti", description="Governed NYC housing repair record assistant")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "serve":
        if args.host not in {"127.0.0.1", "localhost", "::1"}:
            raise SystemExit("Shakti binds to loopback only")
        uvicorn.run("shakti_seva.server:app", host=args.host, port=args.port, reload=False)
        return 0
